...
import os, shutil, random
a = pd.read_csv('../../data/base/Annotations/label.csv', header=None)
b = pd.read_csv('../../data/web/Annotations/skirt_length_labels.csv', header=None)
c = pd.concat([a, b], axis=0)
c.to_csv('../../data/base/Annotations/label_new.csv', header = None, index = None)
    ...
    for path, label in path_label:
        ...
        if train_count < n+1:
            shutil.copy(src_path,
                        os.path.join('../../data/base/Images', task))
        train_count += 1


# Read label.csv
# For each task, make folders, and copy picture to corresponding foldersa
...
mkdir_if_not_exist(['../../data/submission'])
mkdir_if_not_exist(['../../data/incres'])
mkdir_if_not_exist(['../../data/nasnet'])
mkdir_if_not_exist(['../../data/fuse_inc_dense'])
mkdir_if_not_exist(['../../data/fuse_inc_dense_incres'])
mkdir_if_not_exist(['../../data/fuse_inc_dense_incres_nas'])
for task, path_label in label_dict.items():
    ...
    for path, label in path_label:
        ...
        if train_count > n * 0.9:
            # shutil.copy(src_path,
            #             os.path.join('../../data/train_valid_3', task, 'train', str(label_index)))
            shutil.copy(src_path,
